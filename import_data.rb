...
class DataImporter
  ...
  def import_row(row)
    # 1. Handle Area Hierarchy
    n0 = escape(row['n0_empresa'])
    n1 = escape(row['n1_diretoria'])
    n2 = escape(row['n2_gerencia'])
    n3 = escape(row['n3_coordenacao'])
    n4 = escape(row['n4_area'])
    ...
    employee_id = if res.num_tuples > 0
                    ...
                    nome = escape(row['nome'])
                    email_corp = escape(row['email_corporativo'])
                    celular = escape(row['celular'])
                    cargo = escape(row['cargo'])
                    funcao = escape(row['funcao'])
                    localidade = escape(row['localidade'])
                    tempo = escape(row['tempo_de_empresa'])
                    genero = escape(row['genero'])
                    geracao = escape(row['geracao'])
    ...
    # 3. Handle Survey
    data_resposta = parse_date(row['Data da Resposta'])
    
    interesse = parse_int(row['Interesse no Cargo'])
    com_interesse = escape(row['Comentários - Interesse no Cargo'])
    
    contribuicao = parse_int(row['Contribuição'])
    com_contribuicao = escape(row['Comentários - Contribuição'])
    
    aprendizado = parse_int(row['Aprendizado e Desenvolvimento'])
    com_aprendizado = escape(row['Comentários - Aprendizado e Desenvolvimento'])
    
    feedback = parse_int(row['Feedback'])
    com_feedback = escape(row['Comentários - Feedback'])
    
    interacao = parse_int(row['Interação com Gestor'])
    com_interacao = escape(row['Comentários - Interação com Gestor'])
    
    clareza = parse_int(row['Clareza sobre Possibilidades de Carreira'])
    com_clareza = escape(row['Comentários - Clareza sobre Possibilidades de Carreira'])
    
    expectativa = parse_int(row['Expectativa de Permanência'])
    com_expectativa = escape(row['Comentários - Expectativa de Permanência'])
    
    enps = parse_int(row['eNPS'])
    com_enps = escape(row['[Aberta] eNPS'])
...
if __FILE__ == $0
  ...
  importer = DataImporter.new(db_config)
  begin
    importer.connect
    importer.import(File.join(__dir__, 'data.csv'))
  rescue PG::Error => e
